...
@DataJpaTest
public class EmailActivationTokenRepositoryIntegrationTest {
    @Autowired
    private TestEntityManager entityManager;

    @Autowired
    private EmailActivationTokenRepository tokenRepository;

    private User user1, user2;

    private EmailActivationToken token1, token2, token3;
    ...
    public void When_deleteAllByUser_Expect_deletedCorrectTokens() {
        ...
        EmailActivationToken token4 = new EmailActivationToken()
                .setUser(user3)
                .setToken(createRandomString())
                .setExpiryDate(Date.from(LocalDateTime.now().plusDays(1).toInstant(ZoneOffset.UTC)));
        EmailActivationToken token5 = new EmailActivationToken()
                .setUser(user3)
                .setToken(createRandomString())
                .setExpiryDate(Date.from(LocalDateTime.now().plusDays(1).toInstant(ZoneOffset.UTC)));
